...
def test_step0():
    res = []
    upload_files(f"{data['host']}", f"{data['remote_user']}", "2222", f"{data['local_path']}/p7zip-full.deb",
                 f"{data['remote_path']}/p7zip-full.deb")
    res.append(ssh_checkout_negative(f"{data['host']}", f"{data['remote_user']}", "2222",
...
def test_step1(make_folders, clear_folders, make_files):
    res1 = ssh_checkout_negative(f"{data['host']}", f"{data['remote_user']}", "2222",
                        "cd {}; 7z a {}/arx1.7z".format(data['folder_in'], data['folder_out']),
                        "Everything is Ok"), "Test1 Fail"
    res2 = ssh_checkout_negative(f"{data['host']}", f"{data['remote_user']}", "2222",
...
def test_step3():
    assert ssh_checkout_negative(f"{data['host']}", f"{data['remote_user']}", "2222",
